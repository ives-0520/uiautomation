...
本框架基于weditor录制，生成uiautomator2格式的python自动化脚本，并通过提示词工程（Prompt Engineering）驱动AI自动生成可维护的自动化代码。框架集成了AI能力，支持AI视觉元素定位、AI断言、自然语言用例生成等功能，适用于Android APP的UI自动化测试。

# AI相关功能

- **AI视觉元素定位**：集成doubao-1.5-ui-tars-250328 ，支持通过APP截图+自然语言描述，自动识别并定位UI元素。AI返回元素文本、类型、相对坐标、置信度、与其他组件关系等丰富信息，极大提升自动化脚本的健壮性和适应性。
...
- **AI辅助用例生成与自然语言脚本**：支持基于AI的自然语言用例描述、自动生成异常场景测试用例及测试数据，推动自动化测试向“零代码”与“智能化”演进。
- **AI执行自然语言脚本**：支持AI直接运行自然语言脚本，并输出测试报告
- **AI分析日志**：集成doubao-seed-1-6-thinking-250615,支持AI分析pytest测试用例执行的日志，溯源报错原因并给出解决方案
...
- **分层设计**：
  - 设备驱动层：连接设备管理，封装设备操作。
  - 元素管理层：统一管理所有页面元素的定位信息，封装通用的元素操作方法。
  - AI能力层：集成ai模型，实现ai定位元素、ai断言、以及ai日志分析。
  - 页面对象层：每个页面或功能对应一个对象类，继承元素定位和操作层，将业务操作步骤方法化。
  - 测试用例层：组织具体的pytest测试用例，调用页面对象类实现业务流程测试。
  - 
...
1. 使用weditor录制操作，生成uiautomator2脚本至`record/record_scripts`目录。
2. 通过提示词工程驱动AI，自动提取元素定位、生成页面对象类和测试用例。
